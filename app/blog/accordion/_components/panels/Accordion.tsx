...
export function Accordion({ panels }: AccordionProps) {
  ...
  const handleClick = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    const clickedPanelIndex = parseInt(
      clickedPanelHeader.getAttribute('data-index') as string
    );
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLButtonElement>) => {
    if (e.key === 'Enter') {
      ...
      const enteredPanelIndex = parseInt(
        enteredPanelHeader.getAttribute('data-index') as string
      );
  ...
  return (
    <div className="flex w-full max-w-xl flex-col gap-y-1 rounded border border-black bg-emerald-100 p-2">
